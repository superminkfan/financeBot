...
public class YearHandler extends AbstractHandler  {
    private static final Logger log = Logger.getLogger(DayHandler.class);
    private final String END_LINE = "\n";

    public YearHandler(Bot bot) {
        super(bot);
    }
    ...
    public String operate(Long chatId, Update update) {
        ...
        SendMessage sendMessage = Bot.doSendMsg(chatId,"yearHandlerMsg1");
        Buttons.setButtonsMain(sendMessage, chatId);

        StringBuilder s = new StringBuilder();

        ArrayList arrayList = HeapWork.getYearStatSpans(chatId);
